...
Extract the `icomoon.*` fonts to /assets/fonts, delete the original `pearson-fa.*` fonts, and rename the new ones to `pearson-fa.*`.

Update this page with the new icon.

Update the tests in /test/spec/icons.js and /test/fixtures/icons/icons.html. Eventually these will be moved to Selenium.
